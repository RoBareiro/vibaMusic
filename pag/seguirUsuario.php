<?PHP
	...
	/*datos del usuario a seguir*/
	$id_usuarioASeguir = $_GET["id_usuario"];
	$imagenUsuario = $_GET["imagen"];
	$nombreUsuario = $_GET["nombreUsuarioASeguir"];
	/*datos del usuario a seguir*/
...
if($resultado = (mysqli_num_rows($datosAccion)) == 1){
			...
			$busquedaQuery = "SELECT id_seguidor, id_seguido FROM sigue_a WHERE id_seguidor = '$id_usuario' AND id_seguido = '$id_usuarioASeguir' AND id_playlist = '-1'";
			...
			if($cantidad = (mysqli_num_rows($sqlEjec)) == 0){
				$estado = 'sigue';
				$queryParaSeguidor = "INSERT INTO sigue_a (id_seguimiento, id_seguidor, id_seguido, estado, id_playlist) VALUES ('','$id_usuario','$id_usuarioASeguir','$estado','-1')";
					$queryAccion = mysqli_query($conexion,$queryParaSeguidor);
					$errores[0] = "<font style='color: green;'>Ahora Sigue a este Usuario</font></br>";		
			}
			else{
				$errores[0] = "<font style='color: red;'>Ya Sigue a este Usuario</font></br>";
					}
			...
			<div class="banner-info">
				<div class="container">
					...
					<div class="opciones bounceIn">
							<a href="buscarUsuario.php" class="btnUsu">VOLVER A BUSCAR</a></br>
					...
					<span><?PHP echo $errores[0]; ?></span>
						...
						<?PHP
							echo "<b><i>" .$nombreUsuario. "</i></b>";	
							echo "</br><img src='" .$imagenUsuario. "' width='30%'></img>";
						?>			
					</br>
					</div>
